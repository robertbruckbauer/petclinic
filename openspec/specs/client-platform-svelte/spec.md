...
Services, stores, the router, and type definitions are TypeScript under `strict` compiler settings, with `verbatimModuleSyntax` forcing `import type` for type-only imports; `.svelte` files and `app.js` are type-checked as JS/Svelte via `checkJs`. `prettier` with `prettier-plugin-svelte` formats every `.js`/`.ts`/`.svelte` file under `src`; there is no ESLint configuration in this module.
