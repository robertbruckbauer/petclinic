...
`/static/**` and every endpoint in `backend-api-basis` (the health checks and the version endpoint) are never subject to authentication, even after the future security plan lands — they must stay reachable for infrastructure/monitoring regardless of the authorization model.
