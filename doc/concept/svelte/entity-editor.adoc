...
The editor component is a Svelte component with reactive state management using runes for efficient state management and change detection.
...
An event callback invoked on successful creation of the entity.
The changed entity is an argument for the callback.

==== `onupdate`

An event callback invoked on successful mutation of the entity.
The changed entity is an argument for the callback.
...
Siehe xref:{relrootdir}/doc/concept/svelte/entity-service.adoc[Entity Service] für mehr Informationen.
