:includedir: ../../..
:relrootdir: ../../..
= Specification for endpoints

== Configuration overview

Backend services use a central configuration file for REST and GraphQL endpoints.
It is only read when the application starts.

.endpoint.properties
[source,text]
----
include::{includedir}/lib/backend-data/src/main/resources/endpoint.properties[]
----

Backend services use the autoconfiguration capabilities provides by _Spring Boot_.

.configuration
[source,java,options="nowrap"]
----
include::{includedir}/lib/backend-data/src/main/java/esy/app/EsyEndpointConfiguration.java[tags=configuration,indent=0]
----

A `@PropertySource` annotation tells Spring where to find the properties file.

A `@ComponentScan` annotation tells Spring where to look for controller components.

A customizer bean configures `Jackson` for reading and writing JSON structures.

.jacksonCustomizer
[source,java,options="nowrap"]
----
include::{includedir}/lib/backend-data/src/main/java/esy/app/EsyEndpointConfiguration.java[tags=jacksonCustomizer,indent=0]
----

The `JsonJpaMapper` class configures the _Jackson Object Mapper_ and provides operations for reading and writing JSON structures.

.JsonJpaMapper
[source,java,options="nowrap"]
----
include::{includedir}/lib/backend-api/src/main/java/esy/rest/JsonJpaMapper.java[tags=configure,indent=0]
----
<1> With the value `false`, it is ensured that during PATCH operations the list from the JSON structure is merged with the list from the database. +
The behavior for lists is thus consistent with the behavior for values.

<2> Disable exception for unknown properties for robust serialization following the tolerant reader pattern.

A configurer bean configures message converters, argument resolvers, formatters, 
CORS, interceptors, path matching or content negotiation.

.webMvcConfigurer
[source,java,options="nowrap"]
----
include::{includedir}/lib/backend-data/src/main/java/esy/app/EsyEndpointConfiguration.java[tags=webMvcConfigurer,indent=0]
----

A customizer bean configures security for static resources and actuator health endpoints for the REST endpoints.

.securityCustomizer
[source,java,options="nowrap"]
----
include::{includedir}/lib/backend-data/src/main/java/esy/app/EsySecurityConfiguration.java[tags=securityCustomizer,indent=0]
----

A customizer bean configures security filters for the REST endpoints.

.securityFilterChain
[source,java,options="nowrap"]
----
include::{includedir}/lib/backend-data/src/main/java/esy/app/EsySecurityConfiguration.java[tags=securityFilterChain,indent=0]
----

A configurer bean configures the behavior of auto-exposed REST endpoints.

.repositoryRestConfigurer
[source,java,options="nowrap"]
----
include::{includedir}/lib/backend-data/src/main/java/esy/app/EsyEndpointConfiguration.java[tags=repositoryRestConfigurer,indent=0]
----

== Implementation details

include::{includedir}/doc/concept/spring/_json-jpa-rest-controller.adoc[leveloffset=+2]

include::{includedir}/doc/concept/spring/_querydsl-repository.adoc[leveloffset=+2]

include::{includedir}/doc/concept/spring/_graphql-controller.adoc[leveloffset=+2]

include::{includedir}/doc/concept/spring/_json-jpa-repository.adoc[leveloffset=+2]

include::{includedir}/doc/concept/spring/_json-jpa-entity.adoc[leveloffset=+2]
