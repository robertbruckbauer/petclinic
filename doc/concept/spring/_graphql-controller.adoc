...
TIP: See the https://docs.spring.io/spring-graphql/reference[Spring GraphQL reference guide] and https://graphql.org/learn/[GraphQL specification] for more information.
...
The method name follows the pattern `all{Entity}` or `all{Entity}By{Criteria}`.
...
This pattern queries through associations without explicit joins (e.g., `pet.owner.id.eq(value)`).
...
class PetGraphqlTest {
    ...
    @Test
    void queryAllPet() {
        final var value = createWithName("Tom") <1>
        ...
        final var data = graphQlTester
                ...
                .execute();
        assertNotNull(data);
        data.path("allPet[0].name") <2>
                .hasValue()
                .entity(String.class)
                .isEqualTo(value.getName());
        data.path("allPet[0].sex") <3>
                .hasValue()
                .entity(Sex.class)
                .isEqualTo(value.getSex());
        data.path("allPet[0].owner.name") <4>
                .hasValue()
                .entity(String.class)
...
<2> Test for the happy path of a String property query.
<3> Test for the happy path of a Enum property query.
<4> Test for the happy path of a relation query.
