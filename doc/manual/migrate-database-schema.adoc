...
Major changes are implemented with a consolidated Liquibase changelog.
The schema and data migration is executed manually directly on the database.
...
TIP: In test stages with CI/CD automation, rollback safety is essential.
...
Major changes are triggered by domain evolution and quality goals, e.g. performance optimization, data consistency improvements or schema normalization.
They are classified as major when they break backward compatibility and prevent safe application rollback without coordinated migration.

Assuming a migration from database version `v1` to `v2`, then 

. Copy Liquibase changesets from directory `v1` to `v2`.
...
TIP: Liquibase will treat the moved changesets as new and attempt to apply them again.
