...
Major changes are implemented with a consolidated Liqibase script set a schema migration executed manually directly on the database.
...
- new tables
- new colums in existing tables
- new constraints in existing tables
- new index for existing tables
...
. Create a script to migrate the database schema using any PostgreSQL database tool.
