...
Add the liquibase script from `resources/liquibase/v1` for the database table with block title `Liquibase`.
Use the include directive to include the xml file.
Use variable `{includedir}` for root path.

Add the java class for the main entity class with block title `Entity`.
Use the include directive and tags to select only relevant properties from the code with their types and annotations.
Use variable `{includedir}` for root path.

Add a separate section for each related entity with the simple class name as the block title.
Use the include directive and tags to select only relevant properties from the code with their types and annotations.
Use variable `{includedir}` for root path.

Add a separate section for each state transfer objects that has a `properties` tag with the simple class name as the block title.
