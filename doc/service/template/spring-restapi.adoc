...
Add the java class for the main entity class with a short description, a source block and title `Entity`.
Use the include directive and tags to select only relevant properties from the code with their types and annotations.
Use the include directive with this tag `properties` in the source block.
Use variable `{includedir}` for root path.
...
Add a separate section for each enum with a short description, a source block and the title `Enum`
Use the include directive with this tag `enumerations` in the source block.
Use variable `{includedir}` for root path.

Add a separate section for each state transfer object with a short description, a source block and the title `Item`.
Use the include directive with this tag `properties` in the source block.
Use variable `{includedir}` for root path.
