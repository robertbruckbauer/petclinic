...
`PATCH` operation::
Describe operations for attributes (@Column) with only the first example regardless of the type of the existing snippets and add a `TIP` annotation with other attributes following the same pattern.
