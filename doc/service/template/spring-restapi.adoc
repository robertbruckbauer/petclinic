...
Add a separate section for each related entity with a source block and the simple class name as the title.
Use the include directive and tags to select only relevant properties from the code with their types and annotations.
Use variable `{includedir}` for root path.

Add a separate section for each state transfer objects that has a `properties` tag with a source block and the simple class name as the title.
Use the include directive with this tag.
Use variable `{includedir}` for root path.
