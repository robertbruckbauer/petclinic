...
NOTE: When querying `allPet` and requesting the `owner` field for each result, consider adding a `@BatchMapping` in `PetGraphqlController` to load all owner entities in a single database query and avoid N+1 problems.
