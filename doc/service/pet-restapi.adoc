...
NOTE: The `sex` field (enum) supports exact-match filtering via the default Querydsl binding (e.g. `?sex=M`). Consider documenting this parameter with an explicit example.
