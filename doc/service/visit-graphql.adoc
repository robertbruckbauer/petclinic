...
The `Visit` type defines properties and collections.
...
The `pet` relation uses batch loading via `@BatchMapping` to efficiently load pets for multiple visits.
Visits without a pet assigned resolve `pet` relation as `null`.

The `vet` relation uses batch loading via `@BatchMapping` to efficiently load vets for multiple visits.
...
The schema uses the custom scalar type `LocalTime` which maps to Java's `java.time.LocalTime` type for representing times of day with format `HH:mm[:ss]` (seconds are optional).

The `Visit` type corresponds to the entity class.
