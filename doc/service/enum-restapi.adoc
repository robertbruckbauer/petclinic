...
The entity class `Enum` extends `JsonJpaEntity` which provides unique identifier and version for optimistic locking.
...
This operation creates a new entity `Enum` for a discriminator.
Response is of type `Enum`.
...
This operation updates an existing entity `Enum` for a discriminator and code.
Response is of type `Enum`.

TIP: This operation only updates existing entities and does not create new ones.
...
This operation returns all persisted entities for a discriminator ordered by code.
Response is a collection of type `Enum`.
...
This operation deletes a single persisted entity `Enum` for a discriminator and code.
Response is of type `Enum`.
