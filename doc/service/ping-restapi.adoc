...
The entity class `Ping` extends `JsonJpaEntity` which provides unique identifier and version for optimistic locking.
...
This operation creates a new entity `Ping` or updates an existing one with the current timestamp.
The operation requires no request body and automatically sets the timestamp to the current time.
Response is of type `Ping`.
...
This operation returns a single persisted entity `Ping`.
