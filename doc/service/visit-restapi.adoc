...
The entity class `Visit` extends `JsonJpaEntity` which provides unique identifier and version for optimistic locking.
...
The response is a collection of type `Visit`.
...
`?pet.owner.id=b3333333-3333-beef-dead-beefdeadbeef`::
Find visits for pets owned by a specific owner
...
This operation returns the associated pet for a visit.
...
This operation returns the associated vet for a visit.
