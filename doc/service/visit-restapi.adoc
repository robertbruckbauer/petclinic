...
Each visit represents a veterinary treatment with a date, time, associated pet, attending veterinarian, text, and a billable flag.
The treatment `date` is required.
The treatment `time` is optional because it only becomes definitive once treatment begins and may therefore be omitted when the visit is created.
...
The vet relationship is optional.
When a visit has no vet assigned, it may not be clear which vet is on duty at the time the visit is created.

The pet relationship is optional.
When a visit has no pet assigned, it serves as a placeholder for a vet's availability on that date.
