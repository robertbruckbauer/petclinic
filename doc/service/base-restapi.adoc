...
See https://docs.spring.io/spring-boot/reference/actuator/endpoints.html[Spring Boot Actuator Endpoints] for more information.
...
This operation reports `Ok` or code 200 when the `VERSION` file is successfully loaded and available for download in the requested format.

This operation reports `Not Found` or code 404 when the `VERSION` file is missing or cannot be read.

This operation reports `Bad Request` or code 400 when the `VERSION` file content is syntactically invalid.
