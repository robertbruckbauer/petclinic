...
Pet names are unique per owner, allowing different owners to have pets with the same name.

NOTE: Verify that the `owner` relation in the Pet entity is correctly documented in all PATCH operations. The entity implementation marks owner with `optional = false`, meaning a pet must always have an owner. Ensure POST and PUT operation examples include the owner relation, and document what happens when attempting PATCH operations that would remove the owner (should fail with 400 Bad Request due to validation constraint).
