...
NOTE: The `contact` field is declared as `contact: String` (nullable) in the GraphQL schema, but the underlying database column has `nullable=false`.
The Java entity initializes `contact` to an empty string and `verify()` does not enforce a non-empty value.
As a result, the API never returns `null` for `contact` — at a minimum it returns an empty string.
If strict optionality semantics are needed, consider changing the schema to `contact: String!`.
