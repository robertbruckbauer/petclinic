...
NOTE: Document additional PATCH operations if snippets exist for born, species, and owner relation updates (check for patch-api-pet-born, patch-api-pet-species, patch-api-pet-owner).
...
NOTE: Verify the Pet entity model shows optional=false for the owner relationship. The documentation states 'Each pet must have an owner' but check if the implementation enforces this constraint properly.
