...
NOTE: The GraphQL schema shows `id` parameter without `!` (non-null marker), but the controller implementation uses `@NonNull`. The schema should be updated to match the implementation: `ownerById(id: ID!): Owner`. Update query.gqls to add the non-null marker.
...
NOTE: The GraphQL schema shows `name` parameter without `!` (non-null marker), but the controller implementation uses `@NonNull`. The schema should be updated to match the implementation: `ownerByName(name: String!): Owner`. Update query.gqls to add the non-null marker.
