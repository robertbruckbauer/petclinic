...
NOTE: The GraphQL schema shows `id` parameter without `!` (non-null marker), but the controller implementation uses `@NonNull`. The schema should be updated to match the implementation: `petById(id: ID!): Pet`. Update query.gqls to add the non-null marker.
