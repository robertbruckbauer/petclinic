...
The schema uses custom scalar type `LocalDate` which maps to Java's `java.time.LocalDate` type for representing calendar dates.

The schema uses custom scalar type `LocalTime` which maps to Java's `java.time.LocalTime` type for representing times of day.
