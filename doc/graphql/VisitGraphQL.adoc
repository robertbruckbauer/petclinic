...
The text attribute is optional because a veterinarian enters the diagnosis at the end of treatment.

The schema uses custom scalar type `LocalDate` which map to Java's `java.time.LocalDate` type for representing calendar dates.

The schema uses custom scalar type `LocalTime` which map to Java's  `java.time.LocalTime` type for representing times of day.
