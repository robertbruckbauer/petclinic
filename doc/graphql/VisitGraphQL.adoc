...
Each visit represents a veterinary treatment with a date, time, associated pet, attending veterinarian, and visit diagnosis text.
Both the pet and vet relationships are optional.

NOTE: The GraphQL schema shows the `date` field as nullable (`date: LocalDate`), but the Java entity has `@NotNull` annotation on the date field. Consider whether the schema should be updated to `date: LocalDate!` to match the entity constraint.

The schema uses custom scalar types `LocalDate` and `LocalTime` which map to Java's `java.time.LocalDate` and `java.time.LocalTime` types for representing calendar dates and times of day.
...
NOTE: The GraphQL schema shows `date` parameter without `!` (non-null marker), but the controller implementation uses `@NonNull`. The schema should be updated to match the implementation: `allVisitAt(date: LocalDate!): [Visit]`. Update query.gqls to add the non-null marker.
...
NOTE: The GraphQL schema shows `id` parameter without `!` (non-null marker), but the controller implementation uses `@NonNull`. The schema should be updated to match the implementation: `allVisitByPetId(id: ID!): [Visit]`. Update query.gqls to add the non-null marker.
...
NOTE: The GraphQL schema shows `id` parameter without `!` (non-null marker), but the controller implementation uses `@NonNull`. The schema should be updated to match the implementation: `allVisitByVetId(id: ID!): [Visit]`. Update query.gqls to add the non-null marker.
