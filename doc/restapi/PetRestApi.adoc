...
This operation reports `Conflict` or code 409 if the name for the given owner already exists, data is incomplete, or is invalid.
...
NOTE: According to the template rules, PATCH operations for attributes (@Column: name, born, species) should show only the first example. Snippets exist for patch-api-pet-born and patch-api-pet-species but per template guidelines, these should NOT be added as the name example already covers attribute updates. However, the owner field is a @ManyToOne relation, and snippet patch-api-pet-owner exists. Per template: "Describe each operation for relations if a snippet exists." Add a separate section for the owner relation update with example after the attribute example.
