...
This operation reports `Conflict` or code 409 if the entity already exists or data is incomplete.
...
This operation reports `Conflict` or code 409 if the data is incomplete or is outdated (invalid ETag).
...
TIP: The description of attributes other than date are omitted for simplicity because they are following the same pattern.
...
This operation reports `Conflict` or code 409 if this relation already exists or the entity is outdated (invalid ETag).
