...
Each visit represents a veterinary appointment with a treatment date and time, associated pet, attending veterinarian and diagnosis text.
Each pet can only have one treatment per date.
...
This operation reports `Conflict` or code 409 if the unique constraint (date, pet_id) would be violated by the update or the entity is outdated (invalid ETag).
