...
Each visit represents a veterinary appointment with a specific date, associated pet, attending veterinarian, and visit notes.

**Important constraints:**
- Each pet can only have one visit per date (unique constraint on date + pet)
- Both pet and vet relationships are optional but typically required for meaningful visits
...
Visit responses include computed fields for UI convenience:
- `ownerItem`: Simplified owner information derived from the pet
- `petItem`: Simplified pet information for selection lists
- `vetItem`: Simplified veterinarian information for selection lists
