...
Add the liquibase script from `resources/liquibase/v1` for the database table with title `Liquibase`.
Use the include directive to include the xml file.
Use variable `{includedir}` for root path.

Add the java class for the entity class with title `Entity`.
Use the include directive and tags to select only relevant properties from the code with their types and annotations.
Use variable `{includedir}` for root path.

Add a separate section for each state transfer objects that has a `properties` tag with simple class name as the title.
