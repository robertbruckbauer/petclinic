...
<2> The method accepts the parent entity and validates it is not null using `@NonNull`.
<3> The parent reference is set.
<4> The method returns `this` for method chaining.
