...
This is the *only* access-control-adjacent code in the backend today — CORS is a browser-enforced boundary, not a substitute for authentication. See the risk table in `doc/arc42/11-risks-and-technical-debt.adoc` for what that means, and `doc/concept/spring/endpoint.adoc` for the implementation guide covering this same configuration.
