...
This is the *only* access-control-adjacent code in the backend today — CORS is a browser-enforced boundary, not a substitute for authentication. See the risk table in `doc/arc42/11-risks-and-technical-debt.adoc` for what that means, and `openspec/specs/backend-security/spec.md` for the normative statement of this same configuration.
