...
- Every screen traces to a `## UI Requirements` entry in a per-entity UI capability spec (`client-ui-owner`, `client-ui-pet`, `client-ui-vet`, `client-ui-visit`, `client-ui-enum`) plus the two shared capabilities, `client-shell` (inventory/behavior) and `client-style` (composition/appearance).
- Every service call traces to a `## REST Requirements` entry in that entity's backend capability spec (`backend-api-owner`, `backend-api-pet`, `backend-api-vet`, `backend-api-visit`, `backend-api-enum`). Each backend capability also has a `## GraphQL Requirements` section describing a real, implemented backend query set — but no shipped client generates GraphQL calls from it yet (see `backend-api-conventions`'s GraphQL Requirements chapter); it's not "unadopted," just unconsumed.
